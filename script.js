document.addEventListener("DOMContentLoaded", () => {
  ...
  function moveYes(x, y) {
    ...
    const move = 80 + tries * 12;
    const tx = (dx / dist) * move;
    const ty = (dy / dist) * move;

    btnYes.style.transform = `translate(${tx}px, ${ty}px) rotate(${(Math.random()*6-3)}deg)`;
    tries++;

    const texts = ["Almost.", "Not yet.", "Hmm…", "Still no.", "Persistent 👀"];
  ...
  btnNo.addEventListener("mouseenter", () => {
    ...
    hint.textContent = ["Are you sure?", "Think again.", "Last chance.", "Okay…"][Math.min(noHover-1,3)];
  });
